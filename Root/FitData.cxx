...
double FitData::NegLogLikelihood(double* par){
  ...
  for(int i_chan=0; i_chan<m_chans.size(); i_chan++){
    ...
    int bin_low=(m_dataHistMap[channel].get())->FindBin(-5);
    int bin_high=(m_dataHistMap[channel].get())->FindBin(5);
...
void FitData::MakeBootStrapTemplates(){
  ...
  for(int i_chan=0; i_chan<m_chans.size(); i_chan++){
    ...
    for(int i_t=0; i_t<m_templateHistsMap[channel].size(); i_t++){
...
void FitData::AutoRebinHistograms(TString& channel, int N_force){
  ...
  for(int i_bin=1; i_bin<=m_templateHistsMap[channel][0]->GetNbinsX(); i_bin++){
    ...
    for(int i=0; i<n_mc_template.size(); i++){
...
double FitData::GetFlavourFraction(TString &channel, int i_flav_pair){
  ...
  for(int i_hist=0; i_hist<m_templateHistsMap[channel].size(); i_hist++){
