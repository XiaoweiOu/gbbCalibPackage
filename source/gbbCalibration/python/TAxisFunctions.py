...
##
## Set the y-axis ranges of a canvas
##
...
##
def SetAxisLabel(can,label = '', axis = 'y') :
    if can.GetPrimitive('pad_top') :
        SetYaxisRanges(can.GetPrimitive('pad_top'),'', 'y')
    from ROOT import TGraph,TH1
    yaxis = 0
    if axis != 'y' and  axis != 'x' :
        print 'Warning: axis needs to be "x" or "y"!'
        print '         Continue doing nothing!'
        return
    for i in can.GetListOfPrimitives() :
        if issubclass(type(i),TGraph) :
            if axis == 'y' :
        ...
        if issubclass(type(i),TH1) :
            ...
            break
    if not yaxis :
        print 'Warning: SetYaxisLabel had no effect. Check that your canvas has plots in it.'
        return
