...
#-----------------------------------------------
def DrawPlotText(xmin, ymax, tagText=None):
  text = TLatex()
  text.SetNDC()
  text.DrawLatex(xmin, ymax, '#font[72]{ATLAS} #font[42]{%s}' % plotText)
  text.DrawLatex(xmin, ymax-0.04, '#font[42]{#scale[0.8]{%s}}' % sqrtText)
  text.DrawLatex(xmin, ymax-0.08, '#font[42]{#scale[0.8]{%s}}' % calibText)
  if tagText:
    text.DrawLatex(xmin, ymax-0.12, '#font[42]{#scale[0.8]{%s}}' % tagText)
...
def Make2DPlot(bin_vals, bin_errs, name, zrange=None):
  ...
  #Add plot labels
  DrawPlotText(0.675, 0.9, name)

  canv.SaveAs(outdir+'2D'+name+'.'+args.ext)
  canv.Close()
...
def Make1DPlot(bin_vals, bin_yerr, name):
  ...
  bin_xerr[0] = 0.5*(fj_bins[0]-fj_low_cut)
  ...
  gr = TGraphAsymmErrors(N,bin_x,bin_vals,bin_xerr,bin_xerr,bin_yerr,bin_yerr)
  gr.SetMarkerSize(0)
  gr.SetFillColor(419)
  ...
  canv.SaveAs(outdir+name+'.'+args.ext)
  del canv
...
def MakeFlavFracPlot(fitResults, nuisPar, prefit=True):
  ...
  flavCheck = False
  FlavList = MyConfig.GetFlavourGroups()
...
def MakeRatioPlots(var,prefit=True,doErr=True,doChi2=False,setLogy=False):
  ...
  for i,region in enumerate(regions):
    ...
    h_data_incl = config.AddHists(h_data_incl, h_data, 'h_Incl_Data')
    ...
    for j,flav in enumerate(FlavList):
      ...
      if not prefit:
        ...
        if doErr:
          ...
          h_fitErr = config.AddHists(h_fitErr, h_temp)
      ...
      h_mcSum_incl = config.AddHists(h_mcSum_incl, h_mc, 'h_Incl_MC')
      if flav.Data() in h_mc_incl:
        h_mc_incl[flav.Data()].Add(h_mc)
      else:
        h_mc_incl[flav.Data()] = h_mc
  ...
  # now stack regions together for inclusive plot
  if len(regions) > 1:

    leg = TLegend(0.84,0.58,0.94,0.88)
    leg.SetBorderSize(0)
    leg.AddEntry(h_data_incl,'Data','epl')

    mcStack_incl = THStack()
    for flav in FlavList:
      leg.AddEntry(h_mc_incl[flav.Data()],flav.Data(),'f')
      mcStack_incl.Add(h_mc_incl[flav.Data()])

    chi2Text=''
    if doChi2:
      # rebinning means data is weighted histogram too
      chi2 = h_data_incl.Chi2Test(h_mcSum_incl,"WW CHI2/NDF");
      chi2Text='#chi^{{2}}/NDF = {:.2f}'.format(chi2)

    pad1.cd()
    config.EnsureNoZeros(h_data)
    h_data_incl.Draw('EP')
    h_data_incl.GetYaxis().SetLabelSize( 0.04 )
    h_data_incl.GetYaxis().SetTitleSize( 0.04 )
    h_data_incl.GetYaxis().SetTitleOffset(1.)
    if setLogy:
      h_data_incl.GetYaxis().SetRangeUser(0.1*mcStack_incl.GetMinimum(),2*h_data_incl.GetMaximum())
    else:
      h_data_incl.GetYaxis().SetRangeUser(0.1*mcStack_incl.GetMinimum(),1.2*h_data_incl.GetMaximum())
    mcStack_incl.Draw('hist same')
    h_data_incl.Draw('EP same') # redraw data so it sits in front
    leg.Draw()
    DrawPlotText(0.6, 0.84, tagText)

    pad2.cd()
    ratio = h_data_incl.Clone('ratio')
    ratio.Divide(h_mcSum_incl)
    ratio.SetLineColor(1)
    ratio.Draw('EP')
    ratio.GetYaxis().SetRangeUser(0.4,1.6)
    ratio.GetYaxis().SetTitle('Data/MC')
    ...
    canv.SaveAs(outdir+'Incl_'+name+'.'+args.ext)
    # finally, clear the canvas for the next histograms
    canv.Clear('D')

  # lastly, compare chi2 values between regions
  if doChi2:
    if args.bins == 'trkjet':
      Make2DPlot(chi2vals, np.array([]), 'chi2_'+name)
    #elif args.bins == 'fatjet':
    #  Make1DPlot(chi2vals, np.array([]), 'chi2_'+name)
...
for var in args.plots:
  if 'SF' == var:
    ...
    for flav in MyConfig.GetFlavourGroups():
    ...
    logy = any(x in var for x in ['jm'])
    MakeRatioPlots(var,prefit=False,doChi2=True,setLogy=logy)
    MakeRatioPlots(var+'_2TAG',prefit=False,doChi2=True,setLogy=logy)
