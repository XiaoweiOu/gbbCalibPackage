...
TGraphAsymmErrors* ScaleFactorCalculator::getTemplateFitUncertToys(bool applyFitCorrection, std::vector<std::shared_ptr<TH1D>> templateHists, TString& region, TString &sys, int rebin){
  ...
  TVectorD randomv(m_config->GetFlavourGroups().size()), nomv(m_config->GetFlavourGroups().size()), eigenval(m_config->GetFlavourGroups().size());
  for(unsigned int i_p=0; i_p<m_config->GetFlavourGroups().size(); i_p++) nomv(i_p)=m_fit_params[region+"_"+sys][i_p];

  TMatrixD eigenvec(m_config->GetFlavourGroups().size(),m_config->GetFlavourGroups().size());

  std::vector<double> nominal;

  TMatrixDSym mat(m_config->GetFlavourGroups().size(),&m_nom_cov_mats[region][0]);
  //mat.UnitMatrix();
  ...
  for( int i_toy=0; i_toy<n_toys; i_toy++){
    ...
    if(i_toy==0){ //nominal rate
      ...
      for(unsigned int i_p=0; i_p<m_config->GetFlavourGroups().size(); i_p++){
...
std::vector<double> ScaleFactorCalculator::getFractionFitUncertToys(bool applyFitCorrection, std::vector<std::shared_ptr<TH1D>> templateHists, TString& region){
  ...
  TVectorD randomv(m_config->GetFlavourGroups().size()), nomv(m_config->GetFlavourGroups().size()), eigenval(m_config->GetFlavourGroups().size());
  for(unsigned int i_p=0; i_p<m_config->GetFlavourGroups().size(); i_p++) nomv(i_p)=m_fit_params[region+"_Nom"][i_p];
  ...
  for(int i_toy=0; i_toy<n_toys; i_toy++){
    ...
    for(int i_f=0; i_f<m_config->GetFlavourGroups().size(); i_f++){
...
TGraphAsymmErrors* ScaleFactorCalculator::getFitUncert(TString& var){
  ...
  for (TString region : regions) {
    ...
    std::vector<double> flavour_norms(m_config->GetFlavourGroups().size(),0.);
...
TGraphAsymmErrors* ScaleFactorCalculator::getFitUncertBTagRate(){
  ...
  for (TString region : regions) {
    ...
    std::vector<double> flavour_norms(m_config->GetFlavourGroups().size(),0.);
    std::vector<double> flavour_norms_posttag(m_config->GetFlavourGroups().size(),0.);
...
TGraphAsymmErrors* ScaleFactorCalculator::getFitUncertBTagRateToys(){
  ...
  TRandom3 r(0);
  TVectorD randomv(m_config->GetFlavourGroups().size()), nomv(m_config->GetFlavourGroups().size()), eigenval(m_config->GetFlavourGroups().size());
  TMatrixDSym mat(m_config->GetFlavourGroups().size());
  TMatrixD eigenvec(m_config->GetFlavourGroups().size(),m_config->GetFlavourGroups().size());
...
TGraphAsymmErrors* ScaleFactorCalculator::getModellingUncert(TString &var, std::vector<TString> &systematics, bool applyFitCorrection, bool isPosttag, bool isEff){
  ...
  std::vector<double> fj_bins = m_config->GetBinning(var);
  std::vector<TString> regions = m_doFitInFatJetPtBins ? m_config->GetFatJetRegions() : m_config->GetDiTrkJetRegions();
  std::vector<TString> flavours = m_config->GetFlavourGroups();
