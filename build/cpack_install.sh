...
# Install the project:
DESTDIR=${DESTDIR}/usr/WorkDir/21.2.83 \
    make -k install/fast

# Remove the .dbg files from being packaged, in case we are in RelWithDebInfo
# build mode:
rm -f ${DESTDIR}/usr/WorkDir/21.2.83/InstallArea/generic/bin/*.dbg
rm -f ${DESTDIR}/usr/WorkDir/21.2.83/InstallArea/generic/lib/*.dbg
